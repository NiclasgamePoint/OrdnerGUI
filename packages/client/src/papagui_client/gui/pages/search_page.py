...
class SearchPage(QWidget):
    ...
    customerActivated = Signal(object)
    folderActivated = Signal(object)
    openFileRequested = Signal(str)
    openPathRequested = Signal(str)
    ...
    def __init__(
        ...
        self.customer_section.activated.connect(self.customerActivated.emit)
        self.folder_section.activated.connect(self.folderActivated.emit)
    ...
    def reset(
        ...
        *,
        ...
        if customers is None:
            ...
            self.set_customers(customers, len(customers), keys=keys, paths=paths)
    ...
    def set_customers(
        ...
        *,
        ...
        for position, customer in enumerate(customers):
            folders = list(customer.folder_paths or ())
            if not folders and customer.folder_path:
                folders.append(customer.folder_path)
            if not folders and customer.projects:
                folders.extend(
            ...
            rows.append(
                ResultRow(
                    ...
                    paths[position]
                    if paths is not None and position < len(paths)
                    else folders[0] if folders else "",
                )
            )
    ...
    def set_folders(self, folders: list[dict], total: int):
        ...
        for folder in folders:
            ...
            payload = folder.get("navigation_key", path)
            rows.append(
                ResultRow(
